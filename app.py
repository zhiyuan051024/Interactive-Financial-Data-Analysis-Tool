...
if mode == "Pull Data from WRDS Database":
    ...
    if st.button("Connect to WRDS and Pull Data"):
        try:
            ...
            conditions = []
            if permno.strip():
                conditions.append(f"permno = {permno.strip()}")
            ...
            st.success(f"Successfully pulled data! Rows: {len(df)}")

        except Exception as e:
            st.error(f"Connection / Query Failed: {str(e)}")
            st.info("Please check your WRDS credentials, VPN, and input validity.")
